...
static int disk_positioni(disk_t *d, u_int zone) {
    u_long pos;
...
static int disk_makezonei(disk_t *d, u_int zone) {
    u_long pos;
    int block = zone / BLOCK_ZONES, i;

    i = block;

    while (d->d_md[i] == NULL && d->d_md[i - 1] == NULL) i--;
...
static int disk_writedescri(disk_t *d, int block) {
    u_long pos;
    if (block == 0) {
...
static int disk_formcodei(char *buf) {
    int i;
