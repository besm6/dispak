...
mpar:
		...
		} else if (!strncmp((char*)art, "ÙÂÏ", 3)) {
			psp.tele = 1;
		} else if (!strncmp((char*)art, "ÊÈ˙", 3)) {
			if (!cp)
				goto mpar;
			while (*cp && !isdigit(*cp))
				++cp;
			...
			if (!psp.phys ||
					...
					(psp.phys >= 0100))
				goto mpar;
		} else if (!strncmp((char*)art, "ÏÂÓ", 3) ||
